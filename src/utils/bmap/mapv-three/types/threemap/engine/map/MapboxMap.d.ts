/** @protected */
export class MapboxMap extends BaseMap {
    static EARTH_RADIUS: number;
    constructor(engine: any, map: any, options: any);
    MERCATOR_LENGTH: number;
    projectionCoordsName: string;
    origin: number[];
    center: number[];
    pitch: number;
    heading: number;
    near: number;
    _far: number;
    fov: number;
    cameraDistance: number;
    _engine: any;
    map: any;
    domContainer: any;
    afterInit(): void;
    getContainerSize(): any[];
    getResolution(): any;
    handleEventClick: (e: any) => void;
    handleEventDblClick: (e: any) => void;
    handleEventRightClick: (e: any) => void;
    handleMapResize: () => void;
    setCenter(center: any): void;
    setZoom(zoom: any): void;
    setHeading(heading: any): void;
    setPitch(pitch: any): void;
    getCenter(): any[];
    getZoom(): any;
    getZoomUnitsByZoom(zoom: any): number;
    getZoomByZoomUnits(zoomUnits: any): number;
    getZoomUnits(): number;
    getPitch(): any;
    getHeading(): any;
    enableControl(): void;
    disableControl(): void;
    zoomIn(): void;
    zoomOut(): void;
    flyTo(params: any): void;
    updateCamera(): void;
    updateMatrixWorld(): void;
    updateProjectionMatrix(): void;
    normalizeMapPoint(input: any): number[];
    normalizeProjectionPointArr(input: any): number[];
    normalizeEarthPointArr(input: any): number[];
    getProjectionCenter(): number[];
    getCameraDistance(): number;
    set far(arg: number);
    get far(): number;
}
import { BaseMap } from "./BaseMap";
